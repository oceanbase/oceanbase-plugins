...
///  copy from ADBC
///  由于ADBC没有处理JDBC中的一些基本列信息，比如 column_size，这里覆盖代码做一下修复。
...
final class ObjectMetadataBuilder implements AutoCloseable {
    ...
    final VarCharVector columnXdbcTypeNames;
    final IntVector columnXdbcColumnSizes;
    final SmallIntVector columnXdbcDecimalDigits;
    final SmallIntVector columnXdbcNullables;
    final IntVector columnXdbcCharOctetLengthes;
    final VarCharVector columnXdbcIsNullables;
    ...
    private int buildColumns(int rowIndex, String catalogName, String dbSchemaName, String tableName)
        ...
        try (final ResultSet rs =
            ...
            while (rs.next()) {
                ...
                final @Nullable String xdbc_type_name = rs.getString(6);
                final int xdbc_column_size = rs.getInt(7);
                final int xdbc_decimal_digits = rs.getInt(9);
                final int xdbc_nullable = rs.getInt(11);
                final @Nullable String xdbc_is_nullable = rs.getString(18);
                ...
                if (xdbc_type_name != null) {
                    columnXdbcTypeNames.setSafe(rowIndex + columnCount, xdbc_type_name.getBytes(StandardCharsets.UTF_8));
                }
                columnXdbcColumnSizes.setSafe(rowIndex + columnCount, xdbc_column_size);
                columnXdbcDecimalDigits.setSafe(rowIndex + columnCount, xdbc_decimal_digits);
                columnXdbcNullables.setSafe(rowIndex + columnCount, xdbc_nullable);
                if (xdbc_is_nullable != null) {
                    columnXdbcIsNullables.setSafe(rowIndex + columnCount, xdbc_is_nullable.getBytes(StandardCharsets.UTF_8));
                }
