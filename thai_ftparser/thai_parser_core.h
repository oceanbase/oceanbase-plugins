...
namespace oceanbase { namespace thai { class ThaiJNIBridge; } }

/**
 * @defgroup ThaiFtParserCore Thai Fulltext Parser Core
 * @brief Core implementation of Thai fulltext parser
 * @{
 */
 ...
 * @details Stores information about a single token extracted from Thai text
 */
struct TokenInfo {
...
/**
 * Thai Fulltext Parser Core Class
 * @details Main implementation class for Thai language fulltext parsing.
 * This class handles the segmentation of Thai text into tokens using JNI
 * to call Java-based Thai segmentation libraries.
 */
...
private:
    // JNI bridge pointer (actual implementation in thai_jni_bridge.h)
    oceanbase::thai::ThaiJNIBridge* jni_bridge_;  // JNI bridge to Java segmentation library (managed by singleton)
    ...
    /**
     * Calculate the character length of a UTF-8 Thai string
     * @param utf8_text The UTF-8 encoded text
     * @return Number of characters (not bytes)
     * @details Thai text uses multi-byte UTF-8 encoding, so byte count
     * and character count are different. This method correctly counts
     * Thai characters.
     */
    int64_t calculate_thai_char_length(const std::string& utf8_text);
    ...
    ObThaiFTParser(const ObThaiFTParser&) = delete;
    ObThaiFTParser& operator=(const ObThaiFTParser&) = delete;
};
...
using ObThaiFTParser = oceanbase::thai::ObThaiFTParser;
