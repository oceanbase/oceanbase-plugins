...
```
このコマンドは gcc 開発環境をインストールします。

> 環境が既に準備されている場合はこのステップをスキップできます
...
make
```
build ディレクトリに libjapanese_ftparser.so ファイルが生成されます。これが動的ライブラリプラグインです。
