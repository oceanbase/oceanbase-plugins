...
/**
 * @defgroup JNIBridge JNI Bridge
 * @brief JNI bridge for calling Java-based segmentation libraries
 * @{
 */
...
/**
 * JNI Bridge Class
 * @details This class manages the JNI bridge to Java-based segmentation libraries.
...
public:
     ...
     */
    explicit JNIBridge(const JNIBridgeConfig& config = JNIBridgeConfig());
...
public:
     ...
     */
    std::shared_ptr<JNIBridge> get_bridge(const JNIBridgeConfig& config = JNIBridgeConfig());
...
private:
    std::shared_ptr<JNIBridge> bridge_;
