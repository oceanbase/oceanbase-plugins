...
struct RuneStr {
  Rune rune;                 /// 当前字符编码
  uint32_t offset;           /// 在当前内存块中的字节偏移量
  uint32_t len;              /// 当前字符的字节个数
  uint32_t unicode_offset;   /// 在当前文字中的字符偏移量，就是第几个字
  uint32_t unicode_length;   /// 当前多少个字
 ...
 */
struct RuneStrLite {
  uint32_t rune; /// UTF8字符编码的值。一个UTF8字符最多4个字节
  uint32_t len;  /// 一个UTF8字符的字节个数。
...
/**
 * 将字符串切成一个个的UTF8字符数组
 * @param[in] s 字符串内存起始位置
 * @param[in] len 字符串字节长度
 * @param[out] runes 解析后的字符数组
 * @return
 *    - true 解析成功
 *    - false 解析失败，比如某个字符不完整
 */
